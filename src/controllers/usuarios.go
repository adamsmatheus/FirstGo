...
func EditarUsuario(w http.ResponseWriter, r *http.Request) {
	...
	if erro = json.Unmarshal(corpoRequisicao, &usuarios); erro != nil {
		respostas.Erro(w, http.StatusBadRequest, erro)
		return
	}

	if erro = usuarios.Preparar("edicao"); erro != nil {
		respostas.Erro(w, http.StatusBadRequest, erro)
		return
	}
	...
	defer db.Close()
	repositorio := repositorios.NovoRepositorioDeUsuarios(db)
	if erro = repositorio.Atualizar(usuarioID, usuarios); erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}
...
// Seguir usuario permite que o outro usuario também o siga
...
func DeixarSeguir(w http.ResponseWriter, r *http.Request) {
	seguidorID, erro := autenticacao.ExtrairUsuarioID(r)
	if erro != nil {
		respostas.Erro(w, http.StatusUnauthorized, erro)
		return
	...
	if seguidorID == usuarioID {
		respostas.Erro(w, http.StatusForbidden, errors.New("Nao pode deixar de seguir voce mesmo!"))
		return
	}
	db, erro := banco.Conectar()
	if erro != nil {
